...
def task_and_time_slot(time_slots):
    ...
    for i in range(episode*time_slots):
...
task_and_time = task_and_time_slot(time_slots)
...
class mobile_device:
  ...
  # l_comp the to denote the time slot when task km (t) has either been processed or dropped, compδm (t) (in time slots) denote the number of time slots that task km (t) will wait for processing if it is placed in the computation queue
    ...
    state = []  #the state array for storing the values per state obersvation
    state_lst = [] #the lst that has all the states
    ...
    def task_offload(self, task_and_time): 
      ...
      # making the lst to store the processing_delay_comp_time 
        for i in range(time_slots):
            ...
            # for the first case we get the load values a zero
            if i == 0:
            ...
            #make the state for the input in the model layers 
            # add  the Qm,n edge in the state funtion
            # the load matrix is dummy now for the one devices 
                state.append(task_and_time[i][2])   
                state.append(processing_delay_comp_time_slots)
                state.append(processing_delay_trans_time_slots)
                state.append(1)
                state.append(1)
        ...
        #choose the action according to equation 22
        



        #according to the action calculate the load values 
...
print(m1.task_offload(task_and_time))
